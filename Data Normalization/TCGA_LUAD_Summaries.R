...
Human_GEM <- read_excel("H:/Human-GEM-main/model/Human-GEM.xlsx")
loc <- 'H:/benchmarking_study/LUAD/LUAD - Repeat/iMAT Reports/'
...
# Save results
loc <- 'H:/benchmarking_study/LUAD/LUAD - Repeat/iMAT Summaries/'
openxlsx::write.xlsx(x = iMATResults, file = paste0(loc,'TCGA_LUAD_Summarised.xlsx'), asTable = T)
...
openxlsx::write.xlsx(x = iMATResultsPathways, file = paste0(loc,'TCGA_LUAD_SummarisedPathways.xlsx'), asTable = T)
