# Plotting number of active reactions (from binary matrix) in reconstructed metabolic models for both control and disease cases.
...
setwd('D:/Projects/Benchmarking Study/iMAT Results/')
...
ggplot(df2[[1]], aes(x=reorder(Method,-NumberOfReactions), y=NumberOfReactions, fill = '#bdd7e7')) + 
  ...
  labs(title = "ROSMAP")
ggsave(paste0('Figures/ROSMAP_Reaction_Figure.tiff'), 
       units="in", width=6.5, height=5.0, dpi=600, compression = 'lzw')


ggplot(df2[[2]], aes(x=reorder(Method,-NumberOfReactions), y=NumberOfReactions)) + 
  geom_bar(stat = "identity", width=0.6) +
  theme_ipsum() +
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5, 
                                   size = 12, hjust = 1))+
  ...
  guides(fill = guide_colorbar(barwidth = 7, barheight = 1,
                               title.position = "top", title.hjust = 0.5)) +
  labs(title = "LUAD")
ggsave(paste0('Figures/LUAD_Reaction_Figure.tiff'), 
       units="in", width=3.5, height=5.0, dpi=600, compression = 'lzw')
